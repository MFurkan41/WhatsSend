...
class WPApp(Ui_MainWindow):
                    ...
                    if(len(fList) != 0):
                        try:
                            ...
                            try:
                                ...
                                warnMessage("Uyarı!",QMessageBox.Warning,"'Mesaj Durumu' kolonundaki veriyi mesajınıza ekleyemezsiniz. Lütfen '{"+ str(max(fList)+1) + "}' değerini siliniz.")
                                return
    ...
    def sendwp(self):
        ...
        self.thread = MesThread(self.plain.toPlainText(),
                                self.numaralar,
                                self.list_of_files,
                                self.apiKey,
                                self.headers
